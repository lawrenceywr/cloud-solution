...
import { executeIpAllocationPlannerWorker } from "../workers/ip-allocation-planner"

function uniqueStrings(values: string[]): string[] {
  return [...new Set(values)]
}

export async function runIpAllocationPlanner(args: {
  input: unknown
  ...
  const workerResult = await executeIpAllocationPlannerWorker({
    ...workflow.sliceInput,
  ...
  if (workerResult.status === "failed") {
    throw new Error(workerResult.errors?.join("; ") ?? "ip-allocation planner failed")
  }
